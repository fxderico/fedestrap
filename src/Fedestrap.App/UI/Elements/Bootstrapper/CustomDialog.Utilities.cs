...
namespace Fedestrap.UI.Elements.Bootstrapper
{
    public partial class CustomDialog
    {
        ...
        private static string? GetFullPath(CustomDialog dialog, string? sourcePath)
        {
            ...
            if (!sourcePath.StartsWith("theme://", StringComparison.OrdinalIgnoreCase))
                return sourcePath;

            string root = Path.GetFullPath(dialog.ThemeDir);
            string relative = sourcePath[8..].Replace('/', Path.DirectorySeparatorChar);
        ...
        private static System.Windows.Media.FontFamily? ResolveFontFamily(CustomDialog dialog, string? value, XElement xmlElement)
        {
            ...
            string spec = value["theme://".Length..];
            string? requestedName = null;

            int hash = spec.LastIndexOf('#');
            if (hash >= 0)
            {
                requestedName = spec[(hash + 1)..].Trim();
                spec = spec[..hash];
            }

            string? path = GetFullPath(dialog, "theme://" + spec);
            if (path == null || !File.Exists(path))
                throw new CustomThemeException("CustomTheme.Errors.ElementAttributeParseError", xmlElement.Name, "FontFamily", "font file");

            ICollection<System.Windows.Media.FontFamily> families;
            try
            {
                families = System.Windows.Media.Fonts.GetFontFamilies(path);
            }
            catch (Exception ex)
            {
                App.Logger?.WriteLine("CustomDialog::ResolveFontFamily", "Could not read " + path + ": " + ex.Message);
                throw new CustomThemeException("CustomTheme.Errors.ElementAttributeParseError", xmlElement.Name, "FontFamily", "font file");
            }
            ...
            {
                foreach (var typefaceName in family.FamilyNames.Values)
                {
                    if (string.Equals(typefaceName, requestedName, StringComparison.OrdinalIgnoreCase))
                        return family;
                }
            }
