...
namespace Fedestrap.UI.ViewModels.Dialogs
{
    public class ChannelListsViewModel : NotifyPropertyChangedViewModel
    {
        private static readonly string[] ChannelsJsonUrls =
        {
            ...
            "https://raw.githubusercontent.com/fxderico/fedestrap-Roblox-Channels/main/Channels.json"
        };

        private static readonly HttpClient _channelsClient = Fedestrap.Utility.VpnHttpClient.Create();
        ...
        public async Task RefreshAsync()
        {
            ...
            try
            {
                ...
                foreach (var url in ChannelsJsonUrls)
                {
                    try
                    {
                        json = await Fedestrap.Utility.Http.GetStringBoundedAsync(_channelsClient, url);
        ...
        private async Task SaveCacheToDiskAsync()
        {
            try
            {
				...
				Dictionary<string, ClientVersion> snapshot;
                lock (_channelInfoCache)
                {
					snapshot = new Dictionary<string, ClientVersion>(_channelInfoCache);
                }
				await Task.Run(() => Fedestrap.Utility.JsonFile.SerializeAtomic(CacheFilePath, snapshot)).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
        ...
        private async Task LoadCacheFromDiskAsync()
        {
            try
            {
				...
				var cache = await Task.Run(() => Fedestrap.Utility.JsonFile.Deserialize<Dictionary<string, ClientVersion>>(CacheFilePath, Fedestrap.Utility.JsonOptions.Tolerant, 8388608)).ConfigureAwait(false);
                if (cache != null)
                {
                    lock (_channelInfoCache)
                    {
        ...
        private async Task SaveCacheMetaAsync()
        {
            try
            {
				...
				CacheMeta snapshot = new CacheMeta { LastUpdatedUtc = _lastCacheUpdate };
				await Task.Run(() => Fedestrap.Utility.JsonFile.SerializeAtomic(CacheMetaFilePath, snapshot)).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
        ...
        private async Task LoadCacheMetaAsync()
        {
            try
            {
				...
				var meta = await Task.Run(() => Fedestrap.Utility.JsonFile.Deserialize<CacheMeta>(CacheMetaFilePath, Fedestrap.Utility.JsonOptions.Tolerant, 1048576)).ConfigureAwait(false);
                if (meta != null)
                {
